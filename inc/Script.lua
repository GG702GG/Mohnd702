...
return "• ** هذا الامر يخص {المدير,المنشئ,Carbon} فقط  \n"
end
...
if not msg.Creator then return "• ** هذا الامر يخص {المنشئ,Carbon} فقط  \n"
end
...
if not msg.Admin then return "• ** هذا الامر يخص {الادمن,المدير,المنشئ,Carbon} فقط  \n"
end
...
if not msg.SudoUser then return "• **هذا الامر يخص {Carbon} فقط  \n" end
...
if (MsgText[1] == "منشن" or MsgText[1] == "تاك" or MsgText[1] == "منشن فن" or MsgText[1] == "تاك فن") then
if not msg.Admin then return "• ** هذا الامر يخص {الادمن,المدير,المنشئ,Carbon} فقط  \n" end
...
if not msg.Director then return "• **هذا الامر يخص {Carbon,المنشئ,المدير} فقط  \n" end
...
if not msg.Creator then return "• **هذا الامر يخص {Carbon,المنشئ} فقط  \n" end
if not MsgText[2] and msg.reply_id then
if redis:get(max..'lock:kara:'..msg.chat_id_) == 'off' then
...
if not msg.SudoUser then return "• **هذا الامر يخص {Carbon,Carbon} فقط  \n" end
if not MsgText[2] and msg.reply_id then 
...
if not msg.Kara then return "• **هذا الامر يخص {المنشئ الاساسي,Carbon,Carbon} فقط  \n" end
if not MsgText[2] and msg.reply_id then
...
if not msg.Kara then return "• **هذا الامر يخص {Carbon,المنشى الاساسي فقط} فقط  \n" end
if not MsgText[2] and msg.reply_id then
...
if (MsgText[1] == "الرابط" or MsgText[1] == "رابط فن") then
...
if not msg.Creator then return "• **هذا الامر يخص {Carbon,المنشئ,المدير} فقط  \n" end
local NumMDER = redis:scard(max..'owners:'..msg.chat_id_)
if NumMDER ==0 then 
...
if not msg.Director then return "• **هذا الامر يخص {Carbon,المنشئ} فقط  \n" end
...
if redis:get(max.."getidstatus"..msg.chat_id_) == "Photo" then
	if data.photos_[0] then 
		...
		sendPhoto(msg.chat_id_,msg.id_,data.photos_[0].sizes_[1].photo_.persistent_id_,'-›   𝚄𝚂𝙴𝚁𝙽𝙰𝙼𝙴 ¦ '..UserNameID..' .\n-›   𝙼𝚂𝙶𝚂 ¦ '..msgs..' .\n-›   𝚂𝚃𝙰𝚂𝚃 ¦ '..msg.TheRank..' .\n-›   𝙸𝙳 ¦ '..msg.sender_user_id_..' .',dl_cb,nil)
		else
...
if MsgText[2] == "ردود فن" 	then return unlock_replay(msg) end
if MsgText[2] == "الاذاعه" 	then return unlock_brod(msg) end
if MsgText[2] == "ايدي فن" 	then return unlock_ID(msg) end
if MsgText[2] == "ترحيب فن" 	then return unlock_Welcome(msg) end
...
if MsgText[2] == "ردود فن" 	then return lock_replay(msg) end
if MsgText[2] == "الاذاعه" 	then return lock_brod(msg) end
if MsgText[2] == "ايدي فن" 	then return lock_ID(msg) end
if MsgText[2] == "ترحيب فن" 	then return lock_Welcome(msg) end
...
if not msg.SudoBase then return"• **هذا الامر يخص {Carbon} فقط  \n" end
...
sendMsg(MsgText[2],0,'• ** تم تعطيل المجموعه بأمر من Commander \n• ** سوف اغادر ...\n✘')
...
return redis:get(max..":TEXT_SUDO") or '• لا توجد كليشه Commander.\n• يمكنك اضافه كليشه من خلال الامر\n       " `ضع كليشه المطور` " \n'
end
...
return "• ** الاذاعه مقفوله من قبل Carbon  ." 
end
...
return "• **هذا الامر يخص {Carbon} فقط  \n" 
end
...
local inline = {{{text="أضِغط هُنا ",url="t.me/CTTTH"}}}
send_key(msg.sender_user_id_,'  [𝑆𝑜𝑢𝑟𝑐𝑒 𝑋 𝑁𝑜𝑟𝑑](t.me/CTTTH)',nil,inline,msg.id_)
return false
end
...
if not msg.SudoBase then return "• للCarbon فقط ." end
...
if not msg.SudoUser then return "• Carbon فقط." end
return io.popen([[
...
◂ المطور: مطور البوت .
━━━━━━━━━━━━
[- Source X Nord](https://t.me/CTTTH) 
]]
end
...
if not msg.Director then return "• **هذا الامر يخص {Carbon,المنشئ,المدير,Carbon} فقط  \n• " end
local text = [[
• اهلا بك عزيزي
- اوامر التسليه :
━━━━━━━━━━━━
...
if MsgText[1]== "م C" then
if not msg.SudoBase then return "•  Carbon فقط لـ  🎖" end
local text = [[     
•  اهلا بك عزيزي Carbon
...
if not msg.Kara then return "• **هذا الامر يخص {المنشئ الاساسي,Carbon,Carbon} فقط ." end
...
sendMsg(msg.chat_id_,msg.id_,"• تم ارسـال رسالتك إلى Carbon\n• سـارد عليك في اقرب وقت ممكن \n• معرف Commander"..SUDO_USER)
...
sendMsg(msg.chat_id_,msg.id_,"•  , عزيزي Commander\n• هذا ليس معرف قناة , حاول مجددا .")
return false
end
end
...
if redis:get(max..'text_sudo:witting'..msg.sender_user_id_) then -- استقبال كليشه Carbon
...
if allgp >= 300 then
sendMsg(msg.chat_id_,msg.id_,'• اهلا عزيزي Commander\n• جاري نشر التوجيه للمجموعات وللمشتركين ...')			
end
for i = 1, #pv do 
...
"احبك احبك احبك.",
"مووووورررر.",
"احبك و احب اهلك مو بس انت!.",
"اهواكك و اموت فيكك .",
"نتيكن؟.",
"مو بس انت تحبني انا بعد احبك.",
"احبك و احب اهلك مو بس انت!.",
}
local bbb = {
"اسمي ["..Bot_Name.."] ي عمري .",
"اسمي ["..Bot_Name.."] .",
"ها ي عيون البوت انت؟.",
"اسمي ["..Bot_Name.."] أمرني؟.",
"معك ["..Bot_Name.."] يرحم اهلك.",
"اسمي ["..Bot_Name.."] م اتوقع صعب .",
}
local bs = {
  "قم قم",
  "اموح لعيونك",
...
elseif not msg.SudoUser and Text==" ايسن" or Text == "ايسن" then
sendMsg(msg.chat_id_,msg.id_,www[math.random(#ttt)])
...
sendMsg(msg.chat_id_,msg.id_,bbb[math.random(#bbb)])
...
‏
  ...
  --=====================================
  elseif Text== "OWNER" or Text== "MYTH" then
if msg.SudoUser then  
return sendMsg(msg.chat_id_,msg.id_,"[Napoleon.](https://t.me/SO_NAP)")
end 
end 
